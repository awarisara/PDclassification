...
for train, test in kfold.split(inputs, targets):
	...
	np.save('X_train_'+str(fold_no)+'.npy',inputs[train])
	np.save('X_test_'+str(fold_no)+'.npy',inputs[test])
	np.save('y_train_'+str(fold_no)+'.npy',targets[train])
	np.save('y_test_'+str(fold_no)+'.npy',targets[test])

	#https://ichi.pro/th/vggnet-phrxm-tensorflow-xon-kar-reiyn-ru-dwy-vgg16-rwm-xyu-dwy-138948360357786
	model = VGG16(weights=None, include_top=False,input_shape=(m,n,1))
	...
	#checkpoint_dir = os.path.dirname("/data/users/fengdcw/30039nocut/vgg/")
	...
	#save entire model
	model.save('VGG16_'+str(fold_no)+'.model')
	# Loads the weights
